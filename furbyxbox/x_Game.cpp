...
void Game::Input()
{
	...
	if(framework->KeyPressed(SDLK_1, true))
		player->SetWeapon(1);
	if(framework->KeyPressed(SDLK_2, true))
		player->SetWeapon(2);

	if(framework->GetJButton(0, JOY_BTTN_BACK)||framework->KeyPressed(SDLK_ESCAPE))
	{
		if(framework->JButtonState(0, JOY_BTTN_BACK)==1||framework->KeyPressed(SDLK_ESCAPE))
			showExitConfirm = !showExitConfirm;
		else if(framework->JButtonState(0, JOY_BTTN_BACK)==0)
			showExitConfirm = false;
	}
	...
	if(framework->KeyPressed(SDLK_LSHIFT, false)||framework->GetJButton(0, JOY_BTTN_LTRIG))
		movSpeed *= 2.0f, LX *= 2.0f, LY *= 2.0f;
	/*if(framework->GetJButton(0, JOY_BTTN_BLACK))
	{
		framework->SCALE + 0.05;
		if(framework->SCALE >1.00)
			framework->SCALE = 1.00;
	}
	if(framework->GetJButton(0, JOY_BTTN_WHITE))
	{
		framework->SCALE - 0.05;
		if(framework->SCALE <0.80)
			framework->SCALE = 0.80;
	}*/
	...
	if(framework->KeyPressed(SDLK_LCTRL, false)|| framework->ButtonPressed(BUTTON_LEFT)||framework->GetJButton(0, JOY_BTTN_RTRIG))
		Shoot();
...
void Game::LoadResources()
{
	...
	if(!Loaded)
	{
		...
		for(i=0; i<MAX_TEXTURES; i++)
		{
			sprintf(filename, "textures\\map1\\tex%d.bmp",i+1);
			...
			if(framework->FileExists(completeFilename))
			{
				...
				sprintf(filename, "textures\\map1\\tex%dDark.bmp", i+1);
	...
	if(!framework->GetNoSound())
	{
		...
		HeartBeat = framework->LoadSound("sounds\\HeartBeat_xbox.ogg");
...
buildNameLbl:
	...
	for(i=0; i<MAX_TEXTURES; i++)
	{
		sprintf(filename, "textures\\map%d\\tex%d.bmp", curMapNr, i+1);
		sprintf(completeFilename, "%s%s", DATA_PREFIX, filename);
		if(framework->FileExists(completeFilename))
		{
			textures[i][0] = framework->LoadImage(filename, false);
			sprintf(filename, "textures\\map%d\\tex%dDark.bmp", curMapNr, i+1);
			textures[i][1] = framework->LoadImage(filename, false);
		}
		else
			break;
